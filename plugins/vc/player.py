...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        ...
        await send_text(f"{emoji.CHECK_MARK_BUTTON} വോയ്സിൽ കയറി ")
    else:
        await send_text(f"{emoji.CROSS_MARK_BUTTON} ഞാൻ പോണ് ..... ഈ കളിക്ക് ഞാൻ ഇല്ല")
        mp.chat_id = None
...
async def resume_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} വീണ്ടും തുടങ്ങി .... ശല്യം...!",
                               quote=False)
...
async def mute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.MUTED_SPEAKER} എന്നെ ആരാടാ mute ആക്കിയെ?")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def unmute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.SPEAKER_MEDIUM_VOLUME} അങ്ങനെ വഴിക്ക് വാ ")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
