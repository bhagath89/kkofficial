...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        print("- വോയ്‌സിൽ കയറി .പാട്ട് ഇട്")
        await record_and_send_opus()
        await gc.stop()
    else:
        print("- അങ്ങനെ പടക്കാകട ഹുദാ ഹവ്വാ")   
...
async def record_and_send_opus():
    ...
    status = await client.send_message(chat_id, "1/3 പകർത്തുന്നു")
    ...
    title = f"[Evidence] {utcnow_readable}"
    thumb = await client.download_media(chat.photo.big_file_id)
    await status.edit_text("3/3 ധാ വന്നു")
